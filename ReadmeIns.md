I would like to create a WordPress Plugin [VAPT CoPilot] from an uploaded `Features List.json` JSON file. The file primarily contains details about the VAPT Vulnerabilities and Tests to perform and Check if the Webiste is protected against the Vulnerabilities. The plugin must adhere to the WordPress Coding Standards and follow the Best Practices as being practices in the industry. We should use Ajax throug out the development of the plugin source files.

I would like to create a WordPress Plugin [VAPT CoPilot] from an uploaded JSON file. The file will primarily contains details about the VAPT Vulnerabilities and Tests to perform and Check if the Webiste is protected against the Vulnerabilities. The plugin must adhere to the WordPress Coding Standards and follow the Best Practices as being practices in the industry. We should use Ajax throug out the development of the plugin source files.

We should use a versioning system like major-minor-patch and consistently bump `patch` with every revision, and should bump `major` and `minor` components as required as well.

I need the plugin to have three Sub-Menus `VAPT Master Dashboard`, `VAPT Workbench`, `Client Admin Dashboard` - under the `VAPT Copilot` Main Menu.
